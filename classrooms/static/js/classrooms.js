...
$(document).ready(function() {
    ...
    $("form.add_classroom").submit(function(event) {
        $.post(addURL, $(this).serialize(), function(data){
            ...
            if (newClassroom.error === "UNIQUE_NAME_VIOLATED") {
                $('#successModal').css('display', 'block').delay(3000).fadeOut('slow');
            } else {
                var newLi = $('<li id ="'+newClassroom.pk+'"><span class="row-content"><span id ="resource-name-'+newClassroom.pk+'" class="list-element">' +newClassroom.fields.name +'</span></span><span class="row-content"> <span id="resource-building-'+newClassroom.pk+'" class="list-element">' +newClassroom.fields.building +'</span> </span><span class="row-content"> <span id="resource-seats-'+newClassroom.pk+'" class="list-element">' +newClassroom.fields.seats +'</span> </span> <span class="row-content"> <span id="resource-relsubjects-'+newClassroom.pk+'" class="list-element">'+newClassroom.relatedSubjects+'</span></span> <span class="edit" onclick="edit_item('+newClassroom.pk+');">&#x1f589;</span><span id ="resource-delete-'+newClassroom.pk+'" class="close" onclick="delete_item('+newClassroom.pk+', \''+newClassroom.fields.name+'\');">&#x232b;</span></li>').hide();
    ...
    $("form.edit_classroom").submit(function(event) {
        $.post(editURL, $(this).serialize(), function(data){
            ...
            if (newClassroom.error === "UNIQUE_NAME_VIOLATED") {
                ...
                $('span#resource-building-'+newClassroom.pk).text(newClassroom.fields.building);
                $('span#resource-seats-'+newClassroom.pk).text(newClassroom.fields.seats);
                $('span#resource-relsubjects-'+newClassroom.pk).text(newClassroom.relatedSubjects);
